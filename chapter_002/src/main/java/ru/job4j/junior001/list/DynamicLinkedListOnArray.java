...
@ThreadSafe
public class DynamicLinkedListOnArray<T> implements Iterable<T> {
   ...
   private int itrEndSellIndx = -1;  // может быть совсем НЕ последняя ячейка массива. Пока делаем ошибку
    ...
    public DynamicLinkedListOnArray() {
   ...
   public class ArrayIterator implements Iterator<T> {
       ...
       int currentBack = DynamicLinkedListOnArray.this.realCurrentSize;
       int expectedModCount = DynamicLinkedListOnArray.this.modCount;
       int currCellPrevValue = DynamicLinkedListOnArray.this.itrEndSellIndx;
       int currCellIndexValue = -1;
       int currCellNextValue = DynamicLinkedListOnArray.this.itrStartCellInd;
       ...
       public boolean hasNext() {
           boolean result = false;
           if (this.current < DynamicLinkedListOnArray.this.realCurrentSize) {
               result = true;
           }
           return result;
       }
       ...
       public T next() {
           ...
           if (this.expectedModCount != DynamicLinkedListOnArray.this.modCount) {
               throw new ConcurrentModificationException();
           }
           result = (T) DynamicLinkedListOnArray.this.objects[this.currCellNextValue][0];
           currCellPrevValue = (int) DynamicLinkedListOnArray.this.objects[this.currCellNextValue][1];
           currCellIndexValue = (int) DynamicLinkedListOnArray.this.objects[this.currCellNextValue][2];
           currCellNextValue = (int) DynamicLinkedListOnArray.this.objects[this.currCellNextValue][3];
           this.current++;
           return (T) result;
       }
       public T nextBack() {
           ...
           result = (T) DynamicLinkedListOnArray.this.objects[this.currCellPrevValue][0];
           currCellIndexValue = (int) DynamicLinkedListOnArray.this.objects[this.currCellPrevValue][2];
           currCellNextValue = (int) DynamicLinkedListOnArray.this.objects[this.currCellPrevValue][3];
           currCellPrevValue = (int) DynamicLinkedListOnArray.this.objects[this.currCellPrevValue][1];
           this.currentBack--;
           return (T) result;
       }
   }
